...
// Consumer читает события из Kafka (interview.build.request).
...
type EventHandler interface {
	HandleInterviewBuildRequest(ctx context.Context, sessionID string, params map[string]interface{}) error
}
...
func (h *consumerGroupHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for {
		...
		case message := <-claim.Messages():
			...
			if event.EventType != "interview.build.request" {
			...
			// Извлекаем session_id и params из payload
			...
			params, ok := event.Payload["params"].(map[string]interface{})
			if !ok {
				h.logger.Error("Invalid params type in payload")
				session.MarkMessage(message, "")
				continue
			}

			h.logger.Info("Received interview build request",
			...
			if h.consumer.eventHandler != nil {
				...
				if err := h.consumer.eventHandler.HandleInterviewBuildRequest(ctx, sessionID, params); err != nil {
					h.logger.Error("Failed to handle interview build request",
						zap.Error(err),
						zap.String("session_id", sessionID),
					)
				}
