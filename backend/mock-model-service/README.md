## mock-model-service — заглушка модели для обработки чатов (будущий marking-service)

`mock-model-service` — это умная заглушка, которая имитирует работу AI-модели для обработки чатов. Сервис читает события из Kafka (`chat.events.out`) и отправляет ответы в Kafka (`chat.events.in`). В будущем это будет `marking-service`, который будет анализировать ответы и генерировать результаты.
