...
class KafkaProducer:
    ...
    def publish(
        self, topic: str, event: KafkaEvent, key: Optional[str] = None
    ) -> None:
        ...
        with self.metrics.kafka_producer_duration.time():
            try:
                ...
                if key is None and "chat_id" in event.payload:
    ...
    def publish_batch(
        self, topic: str, events: List[KafkaEvent], key: Optional[str] = None
    ) -> None:
        """Publish batch of events to Kafka topic"""
        for event in events:
